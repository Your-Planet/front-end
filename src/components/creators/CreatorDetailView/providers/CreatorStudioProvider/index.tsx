...
export interface CreatorStudioContextProps {
...
const CreatorStudioContext = createContext<CreatorStudioContextProps | undefined>(undefined);

export const useCreatorStudio = () => useContext(CreatorStudioContext);

interface CreatorStudioProviderProps {
	children: ReactNode;
}

function CreatorStudioProvider({ children }: CreatorStudioProviderProps) {
	// TODO @김현규 API 연동
	const contextValue: CreatorStudioContextProps = useMemo(
		() => ({
	...
	return <CreatorStudioContext.Provider value={contextValue}>{children}</CreatorStudioContext.Provider>;
}

export default CreatorStudioProvider;
